package com.zemleroica.rtsmod.gui;

import java.util.ArrayList;
import java.util.Collection;
import java.util.List;

import com.zemleroica.rtsmod.common.TeamManager;
import com.zemleroica.rtsmod.data.Team;

import net.minecraft.client.gui.GuiGraphics;
import net.minecraft.client.gui.screens.Screen;
import net.minecraft.client.gui.components.Button;
import net.minecraft.network.chat.Component;

public class TeamSelectScreen extends Screen {
    private List<Team> teams = new ArrayList<>();
    private Team selectedTeam;

    public TeamSelectScreen() {
        super(Component.literal("Select Team"));
    }

    @Override
    protected void init() {
        this.clearWidgets();
        Collection<Team> allTeams = TeamManager.getAllTeams();
        this.teams.addAll(allTeams);

        int startY = 80;
        int buttonWidth = 200;
        int buttonHeight = 20;

        for (int i = 0; i < this.teams.size(); i++) {
            Team team = this.teams.get(i);
            this.addRenderableWidget(Button.builder(Component.literal(team.name), button -> {
                selectedTeam = team;
            }).bounds(this.width / 2 - buttonWidth / 2, startY + (i * (buttonHeight + 10)), buttonWidth, buttonHeight)
                .build());
        }

        this.addRenderableWidget(Button.builder(Component.literal("Create New Team"), button -> {
        }).bounds(this.width / 2 - 100, this.height - 70, 200, 20).build());

        this.addRenderableWidget(Button.builder(Component.literal("Confirm"), button -> {
            if (this.selectedTeam != null) {
                this.onClose();
            }
        }).bounds(this.width / 2 - 100, this.height - 40, 200, 20).build());
    }

    @Override
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        this.renderBackground(graphics);
        graphics.drawCenteredString(this.font, this.title, this.width / 2, 20, 0xFFFFFF);
        if (this.selectedTeam != null) {
            graphics.drawString(this.font, "Selected: " + this.selectedTeam.name, 20, 50, 0xAAAAAA);
        }
        super.render(graphics, mouseX, mouseY, partialTick);
    }

    @Override
    public boolean shouldCloseOnEsc() {
        return false;
    }
}